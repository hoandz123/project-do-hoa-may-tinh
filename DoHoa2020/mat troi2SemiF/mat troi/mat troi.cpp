...
//khai báo bán kính các hành tinh
#define ktrang  0.05
#define kthuy  0.1
#define khoa  0.15
#define kkim  0.2
...
//khai báo biến góc quay của từng hành tinh
...
//-------------------------------------
    ...
    glRotatef(thuy, 0.0, 1.0, 0.0); // cho Sao Thủy quay quanh góc tọa độ mặt trời trước rồi mới vẽ
    glPopMatrix();//quay về góc tọa độ củ là mặt trời
    glPushMatrix();// lưu góc tọa độ của mặt trời

    glTranslatef(2.0, 0.0, 0.0); // dời góc tọa độ của mtroi đến vị trí mới để vẽ sao thủy từ vị trí tâm mặt trời
    glColor3f(1.0, 1.0, 1.0); // thiết lập màu trắng cho sao thuy 
    glutSolidSphere(kthuy, 50, 50); // vẽ sao thủy
    glPopMatrix();//quay về góc tọa độ củ là mặt trời
    glPushMatrix();// lưu góc tọa độ

    glRotatef(kim, 0.0, 1.0, 0.0); // Sao kim quay quanh mặt trời
    //số 3.0 là k/cach tu mtroi đến s.kim trừ đi s.thủy là 2.0 thì k/cach s.thủy -> s.kim là 1
    ...
    //chú ý không lưu lại tọa đồ ban đầu, vì để vẽ tiếp mặt trăng
...
void spinDisplay(void)
{
   ...
   hiện việc vẽ lại */
    ...
    //sao kim quay 225 ngày ở trái đất thì hết 1 vòng mặt trời <=> 360/225=1.6
    kim = kim + 1.6;
    if (kim >= 360) kim = 0;

    //sao thủy quay 88 ngày ở trái đất thì hết 1 vòng mặt trời <=> 360/88=4.09
    thuy = thuy + 4.09;
    if (thuy >= 360) thuy = 0;

    //trái đất quay 365.24 ngày thì hết 1 vòng mặt trời <=> 360/365.24=0.98
    dat = dat + 0.98;
    if (dat >= 360) dat = 0;
    hoa = hoa + 0.52;
    if (hoa >= 360) hoa = 0;
