...
public:
  ...
  void MakeBullet(SDL_Surface* des, const int& x_limit, const int& y_limit);// hàm bắn đạn ra
  void Reset(const int& xboder);//reset đối tượng hiểm họa 
  void ResetBullet(BulletObject* p_bullet);//reset viên đạn
private:
  int x_val_;
  int y_val_;
